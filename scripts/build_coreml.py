...
def gen_data(n=20000):
    ...
    s = X.copy()
    sleep_ok = (s[:, 1] >= 7.0).astype(float)
    q_ok = (s[:, 2] >= 60).astype(float)
    hrv_ok = (s[:, 5] > 40).astype(float)
    hi_stress = (s[:, 9] >= 2).astype(float)
    hi_screen = (s[:, 6] > 480).astype(float)
    hi_cal = (s[:, 7] >= 2).astype(float)
    lo_sleep = (s[:, 1] < 6.0).astype(float)
    good_en = (s[:, 8] >= 3).astype(float)
    ...
    focus = np.clip(np.round(55.0 + 10*sleep_ok + 5*good_en - 15*hi_screen - 10*hi_cal + 5*q_ok + np.random.normal(0,8,n)), 0, 100)
...
m.fit(Xn[:split], y[:split], epochs=50, batch_size=64, verbose=0,
...
spec = ct.models.neural_network.NeuralNetworkBuilder(
    input_features=[("features", datatypes.Array(N_FEATURES))],
    output_features=[("output", datatypes.Array(3))]
).spec
builder = ct.models.neural_network.NeuralNetworkBuilder(spec=spec)
builder.add_inner_product(name="dense1", W=W1.T.astype(np.float32), b=b1.astype(np.float32),
    input_channels=N_FEATURES, output_channels=32, input_name="features", output_name="dense1_out", has_bias=True)
builder.add_batchnorm(name="bn1", channels=32, gamma=w[2].astype(np.float32), beta=w[3].astype(np.float32),
    mean=w[4].astype(np.float32), variance=w[5].astype(np.float32), input_name="dense1_out", output_name="bn1_out")
builder.add_activation(name="relu1", non_linearity="RELU", input_name="bn1_out", output_name="relu1_out")
builder.add_inner_product(name="dense2", W=w[6].T.astype(np.float32), b=w[7].astype(np.float32),
    input_channels=32, output_channels=16, input_name="relu1_out", output_name="dense2_out", has_bias=True)
builder.add_batchnorm(name="bn2", channels=16, gamma=w[8].astype(np.float32), beta=w[9].astype(np.float32),
    mean=w[10].astype(np.float32), variance=w[11].astype(np.float32), input_name="dense2_out", output_name="bn2_out")
builder.add_activation(name="relu2", non_linearity="RELU", input_name="bn2_out", output_name="relu2_out")
builder.add_inner_product(name="output_dense", W=w[12].T.astype(np.float32), b=w[13].astype(np.float32),
    input_channels=16, output_channels=3, input_name="relu2_out", output_name="output", has_bias=True)
